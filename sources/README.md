...
| File | What it is |
|---|---|
| `xiaohongshu_harness_next_transcript.md` | Full transcript of the 39-card Xiaohongshu essay "Harness 之后，Agent+Robot 下一站是什么？" by 具身RL日记 (text only; the original images are not redistributed). |
| `code_policy_self_evolving_agents.pptx` | The 25-slide Code-as-Policy → self-evolving robot agent deck provided with the task. |
| `code_policy_deck_extracted.md` | Text, tables and speaker notes extracted from the deck with python-pptx. |
...
| `zhihu_robocurve_gpt6_astra_transcript.md` | Transcript of 罗清雨's Zhihu article "RoboCurve：GPT-6 Astra 直接控制机器人" (third-party RoboCurve test: GPT-6 Astra controlling YAM arms via EEF waypoint tool calls); interpreted in notes/44. |
